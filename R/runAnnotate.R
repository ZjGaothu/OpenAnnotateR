...
#' @return taskID
#' @export runAnnotate
runAnnotate <- function(file_path, species,protocol,perbase)
{
  cell_type <- 1
  if(is.character(file_path)){
    ...
    url = sprintf('http://166.111.5.185:80/openness/anno/info/stat/celltp_%s_%s.txt',speciesDict[as.character(ask_species)],protocolDict[protocol])
    result = getURL(url)
    ...
    taskID <- generateID()
    url_post <- 'http://166.111.5.185:80/openness/anno/phpa/stepu.php'
