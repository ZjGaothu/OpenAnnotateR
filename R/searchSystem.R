...
#' @export searchSystem
#' @examples searchSystem(1,1,'Urinary')
searchSystem <- function(protocol,species,keyword)
{
  ...
  if(species == 3 & protocol == 3){
    ...
    return(0)}
  url = sprintf('http://166.111.5.185:80/openness/anno/info/meta/headFiles/%s_%s.txt',speciesDict[as.character(species)],protocolDict[protocol])
  result = getURL(url)
  result = str_split(result,'\n',simplify = T)
  result = str_split(result,'\t',simplify = T)
  count = 0
  for(i in 1: (dim(result)[1]-1)){
    if(grepl(keyword,result[i,1]) == TRUE)
    {
      count = count + 1
      print(paste(c(as.character(count) , " - " ,result[i,1],' - ' ,result[i,3]),collapse = ''))
    }
  }
}
